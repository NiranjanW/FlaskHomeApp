...
original_author="NiranjanW"
original_name="flaskhomeapp"
original_urlname="FlaskHomeApp"
original_description="Awesome flaskhomeapp created by NiranjanW" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/flaskhomeapp "${name}"
...
then
    ...
    echo "then run 'flaskhomeapp' or 'python -m flaskhomeapp'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'flaskhomeapp' or 'python -m flaskhomeapp' or 'poetry run flaskhomeapp'"
fi
