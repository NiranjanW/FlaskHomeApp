...
- 🎯 Entry points to execute your program using `python -m <flaskhomeapp>` or `$ flaskhomeapp` with basic CLI argument parsing.
...
---
# flaskhomeapp Flask Application

Awesome flaskhomeapp created by NiranjanW
...
```bash
git clone https://github.com/NiranjanW/FlaskHomeApp flaskhomeapp
cd flaskhomeapp
make install
```
...
```bash
flaskhomeapp create-db   # run once
flaskhomeapp populate-db  # run once (optional)
flaskhomeapp add-user -u admin -p 1234  # ads a user
flaskhomeapp run
```
